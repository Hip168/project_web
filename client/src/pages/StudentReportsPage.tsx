      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Quản lý và theo dõi các hoạt động đã tham gia
          </p>
        </div>
        <ActionButton
          onClick={() => setIsModalOpen(true)}
          className="px-4 py-2 bg-indigo-600 text-white rounded-lg transition-colors flex items-center"
          colorOnHover="#432dd7"
        >
        ...
        onSubmit={async ({ selectedActivityId, proof }) => {
          try {
            ...
            const response = await api.get(
              `/participations/student/${user?.profile?.id}`
            );
            setParticipations(response.data);
          } catch (error) {
